...
DESCRIPCIÓN.
	...
	de vulnerabilidad medida como estar por abajo o por encima de la mediana del
	IPM	en 2015 (pre tratamiento).
...
global dir_estimaciones "${output}/Modelos IPM/_heterogeneidad vulnerabilidad/estimaciones"
global output_path "${output}/Modelos IPM/_heterogeneidad vulnerabilidad/graficos"
...
* Iterar sobre modelos V0 y V1 (Menos vulnerables y Más vulnerables respectivamente)
forvalues numV=0/1{
	* IPM 1
	estimates use "${dir_estimaciones}/esp${E}-${modelo}-IPM 1-vuln`numV'-PP.ster"
	...
	* IPM 2
	estimates use "${dir_estimaciones}/esp${E}-${modelo}-IPM 2-vuln`numV'-PP.ster"
	qui estat simple, estore(v`numV'_1) level(90)
...
coefplot matrix(M3E4V1), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) ///
	name(a, replace) title("Más vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D) 
	
	
coefplot matrix(M3E4V0), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) name(b, replace) ///
...
coefplot matrix(M3E5V1), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) name(c, replace) ///
...
coefplot matrix(M3E5V0), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) name(d, replace) ///
	title("Menos vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D)
...
graph export "${output_path}/coefplot-IPM-vulnerabilidad-model3.jpg", replace	
...
coefplot matrix(M4E4V1), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) ///
	name(a4, replace) title("Más vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D) 
...
coefplot matrix(M4E4V0), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) ///
	name(b4, replace) title("Menos vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D)
...
coefplot matrix(M4E51), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) ///
	name(c4, replace) title("Más vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D)
...
coefplot matrix(M4E50), ci((2 3)) vert yline(0, lpattern(dash)) aspect(1.5) ///
	name(d4, replace) title("Menos vulnerables") ///
	ytitle("ATT") mlabel(@b) format("%9.3g") scheme(s1color) ///
	ciopts(lcolor(stc1) recast(rcap)) ///
	mlabcolor(stc1) mcolor(stc1) msymbol(D)
...
graph export "${output_path}/coefplot-IPM-vulnerabilidad-model4.jpg", replace		
