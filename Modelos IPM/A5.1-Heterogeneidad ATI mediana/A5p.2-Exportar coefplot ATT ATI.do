...
DESCRIPCIÓN:
	...
	del IPM de aquellos CUBs de acuerdo a la categoría de ATI recibida. 
...
global dir_estimaciones "${projectfolder}/Output/Estimación_modelos/estimaciones/_heterogeneidad ati"
										
use "${datafolder}/base_estimaciones_gus.dta", clear
global output "${projectfolder}/Output/Graficos/_heterogeneidad ati"
cd "${projectfolder}/Output/Graficos/_heterogeneidad ati"
...
coefplot matrix(M${n_model}E`E'ati0), ci((2 3)) vert yline(0, lpattern(dash)) ///
	...
	mlabcolor(stc6) mcolor(stc6) msymbol(D) aspect(1.4) name(g0, replace)
* ATI = 1	
...
graph export "${output}/heterogeneidadATI_model${n_model}esp`E'.jpg", replace
