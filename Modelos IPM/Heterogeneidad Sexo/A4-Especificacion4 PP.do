...
*-------------------------------------------------------------------------------
**# Estimar modelos 1 y 2 para hombres (sexo_titular==0)
*-------------------------------------------------------------------------------
global S=0
global SEX = "sexo${S}"
preserve
...
*************************************
	...
	* Revisar cohortes de tratamiento 
	qui tab year_t_AAI_Nada if (ejecucion_PNIS_prima==0 | ejecucion_PNIS_prima==1)
	if r(r)>1{
		* Iterar sobre IPM - - - - - - - - - - - - - - - - - - - -
		forvalues ipm=1/2{
			global tipm="IPM `ipm'"
			di "> IPM `ipm'"
			global estimate_file_name "${ESP}-${modelo}-${tipm}-${SEX}"
			di "$estimate_file_name"
			
			csdid IPM_`ipm' if ///
				ejecucion_PNIS_prima == 0 | ///
		...
		tab year_t_AAI_Nada if (ejecucion_PNIS_prima==0 | ejecucion_PNIS_prima==1) & ///
		year==2017
	}
...
*************************************	
	...
	global n_modelo 2
	global modelo "model${n_modelo}"
	qui tab year_t_SA_AAI if ejecucion_PNIS_prima==1 | ejecucion_PNIS_prima==2
	if r(r)>1{
	* Iterar sobre IPM - - - - - - - - - - - - - - - - - - - -
		forvalues ipm=1/2{
			global tipm="IPM `ipm'"
		...
		di "No hay suficientes observaciones""
		tab year_t_SA_AAI if (ejecucion_PNIS_prima==1 | ejecucion_PNIS_prima==2) & ///
		year==2017
	}
...
*-------------------------------------------------------------------------------
**# Estimar modelos 1 y 2 para mujeres (sexo_titular==1)
*-------------------------------------------------------------------------------
global S=1
global SEX = "sexo${S}"
preserve
* Mantener solo observaciones para mujeres
	keep if sexo_titular ==${S}
